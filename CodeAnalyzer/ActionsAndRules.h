...
namespace CodeAnalysis
{
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			ASTNode* top = p_Repos->scopeStack().top();
			int stLength = ((*pTc)[pTc->find("using")]).length();
			...
			GrammarHelper::showParse("using ", *pTc);
		}
	};
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			ASTNode* top = p_Repos->scopeStack().top();
			int stLength = ((*pTc)[pTc->find("typedef")]).length();
			std::string name = (*pTc)[pTc->find("typedef")];
			top->type_ = "typedef";
			top->name_ = name;
			top->package_ = p_Repos->package();
			p_Repos->AST().typeMap()[name] = top;
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			GrammarHelper::showParse("enum ", *pTc);
		}
	};
	///////////////////////////////////////////////////////////////
