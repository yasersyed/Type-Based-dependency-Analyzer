...
{
	std::cout << "\n Requirement 2: Using standard library's streams for all I/O operations and memory management \n -------------------------------------------------------------------------------------------- \n\n";
	std::cout << "\n Requirement 3: Provided packages Type Table, Type Analysis, Dependency Analyzer, Strong Components, NoSqlDb, TestExecutive(CodeAnalyzer) \n ---------------------------------------------------------------------------------------------------------------------------------------- \n\n";
	std::string DBOutput = "DBOutput.xml", SCOutput = "DefaultSC.xml";
	using namespace CodeAnalysis;
	std::cout << "\n Requirment 8: Processing Command Line arguments, looking for Path to directory tree, file patterns to match, specification of XML file \n------------------------------------------------------------------------------------------------------------------------------------- \n\n";
	std::string Varg = argv[argc - 1], Varg2 = argv[argc - 2];
	if (argv[argc - 1] != nullptr && argv[argc -2] !=nullptr) {
		if (Varg == "-a" || Varg == " ")
			std::cout << "\n Command line file name not found, SC Will be stored in default file: " << SCOutput << " \n ";
		else{
			std::cout << "\n Processing command line and extracting results file name : " << argv[argc - 1] << " \n ";
			SCOutput = argv[argc - 1];}
	}
	if(Varg2 == "*.cpp" || Varg2 == "*.h" || Varg2 == "*.cs" || Varg2 == "*-a")
		std::cout << "\n Command line file name not found, DB Will be stored in default file: " << DBOutput << " \n ";
	else{
		std::cout << "\n Processing command line and extracting results file name : " << argv[argc - 2] << " \n ";
		DBOutput = argv[argc - 2];}
	...
	try {
		...
		NoSqlDb<std::string> db = da.findDependency(newtable, vec, DBOutput);
		graph g;
		g.createGraph(db,SCOutput);
		std::cout << "\n Requirement 9: This is an automated test suite demonstrating requirements of this project \n -------------------------------------------------------------------------------------------- \n\n";
	}
