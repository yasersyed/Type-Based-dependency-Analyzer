...
namespace GraphLib
{
	...
	public:
		...
		void createGraph(NoSqlDb<std::string> db,std::string outp);
		size_t findVertexIndexById(size_t id);
		Vertex<V, E> findVertexIndexByName(V name);
		size_t size();
		template<typename F>
	...
	template<typename V, typename E>
	Vertex<V,E> Graph<V, E>::findVertexIndexByName(V name)
	{
	...
	//-----< Creating graph >------------------------
	template<typename V, typename E>
	inline void Graph<V, E>::createGraph(NoSqlDb<std::string> db,std::string out) {
		Graph<V, E> egraph;
		Keys k = db.keys();
		for (Key ki : k)
		{
			Vertex<std::string, std::string> vert(ki);
			egraph.addVertex(vert);
		}
		for (Key ki : k) {
			std::vector<std::string> edgeG = db.value(ki).children;
				for (auto it : edgeG)
				{
					if (egraph.findVertexIndexByName(it).value() != "") {
						Vertex<std::string, std::string> vertp = egraph.findVertexIndexByName(ki);
						Vertex<std::string, std::string> vertc = egraph.findVertexIndexByName(it);
						std::stringstream ss, ss1;
						ss << vertp.id();
						ss1 << vertc.id();
						std::string eval = ss.str() + "->" + ss1.str();
						egraph.addEdge(eval, vertp, vertc);
					}
				}
		}
		std::cout << "\n Displaying graph created with analyzed dependencies \n ------------------------------------------------------- \n";
		display::show(egraph,out);
	}
	///////////////////////////////////////////////////////////////
	// Display class
	...
	public:
		...
		static void show(graph& g,std::string out)
		{
			std::unordered_map < std::string, std::vector<std::string>> mt;
			graph::iterator iter = g.begin();
			while (iter != g.end())
			{
				vertex v = *iter;
				...
				for (size_t i = 0; i<v.size(); ++i)
				{
					...
					vec.push_back(g[edge.first].value().c_str());
				}
				mt.emplace((v.value()).c_str(),vec);
				++iter;
				std::cout << "\n";
			}
			StrongComp t;
			std::string xmlOutput =t.executiveSC(mt);
			try {
				std::ofstream file(out);
				file << xmlOutput;
				std::cout << "\n Requirement 7 - Storing StrongComponents result in " << out << " File \n";
				file.close();
			}
